...
exports.loadDataCreateReport = async (req, res) => {
  ...
  if (localID === null) {
    ...
    const removals = await Removals.find({ localID,
      datetimeRemoval: {
...
exports.createReport = async (req, res) => {
  ...
  const local = await Locals.findOne({ _id: localID })
  const datetimeFinish = new Date(req.body.datetimeFinish.replace(/['"]+/g, ''))
  ...
  const removalsActual = await Removals.find({ localID,
    datetimeRemoval: {
      $lt: datetimeFinish,
      $gt: datetimeInit
    },
  ...
  const removalsPrev = await Removals.find({ localID,
    datetimeRemoval: {
  ...
  const removalsHistoric = await Removals.find({ localID,
    datetimeRemoval: {
      $lt: datetimeInit
    },
    status: { $ne: 'DELETED' }
  ...
  if (datetimeInit.getMonth() === datetimeFinish.getMonth()) {
    ...
    month = months[datetimeInit.getMonth()] + ' ~ ' + months[datetimeFinish.getMonth()]
  }
  ...
  var data = [
    ...
    { materialID: 'ELECTRONICS', name: 'Electrónicos', quantity: 0, prev: 0, x: 0 },
  ...
  data.map((element) => {
    let temp = ecoData.filter(material => material.materialID === element.materialID)[0]
    payload.push({
      ...
      data: temp.savesPerKilogram.map(spk => {
        return {
          name: spk.name,
          quantity: Math.round(spk.quantity * element.quantity) + ' ' + spk.unity,
  ...
  payload.filter(element => element.v).forEach(data => {
  ...
  total.forEach(element => {
    if (element.ID === 'PETROL') {
      metadata.push('Con el petróleo ahorrado un vehículo puede andar ' + Math.round(element.q * 6.8 * 100) / 100 + 'Km (a 6.8 Km/L)')
    }
    if (element.ID === 'ENERGY') {
      metadata.push('Con la energía ahorrada una casa puede funcionar ' + Math.round(element.q / 22.47 * 100) / 100 + ' días (a 22.47 kWatt/día)')
    }
  })

  const outlabeledPie = "https://quickchart.io/chart?width=280&height=240&c={type:'doughnut',data:{labels:[" +
    labels.substr(0, labels.length - 1) + "], datasets:[{label:'Materiales',data:[" +
    dataActual.substr(0, dataActual.length - 1) + '], backgroundColor:[' +
    colors.substr(0, colors.length - 1) +
    ']}]}, options: {legend: {labels: {fontSize: 9, boxWidth: 10}}}}'

  const acumulated = "https://quickchart.io/chart?width=280&height=240&c={type:'bar',data:{labels:[" +
    labels.substr(0, labels.length - 1) + "], datasets:[{label:'Actual',data:[" +
    dataActual.substr(0, dataActual.length - 1) + "],backgroundColor: 'CORNFLOWERBLUE'},{label:'Anterior',data:[" + dataPrev.substr(0, dataPrev.length - 1) +
    "],backgroundColor: 'lightgreen'},{type:'line',fill:false, label:'Promedio histórico',data:[" + dataMedia.substr(0, dataMedia.length - 1) +
  ...
  const report = await Reports.create({ materials: data, localID, datetimeInit, datetimeFinish, month })
